...
class project(osv.osv):
    ...
    def _get_alias_models(self, cr, uid, context=None):
        ...
        return [('project.task', "Line Items")]
    ...
    _columns = {
        'parent_customer_name':fields.char('Parent Customer Name',    required=True),
        ...
        'date_of_creation': fields.date('Date Of Creation', select=True, copy=False),
        ...
        'type_ids': fields.many2many('project.task.type', 'project_task_type_rel', 'project_id', 'type_id', 'Line Items Stages', states={'close':[('readonly',True)], 'cancelled':[('readonly',True)]}),
        'task_count': fields.function(_task_count, type='integer', string="Line Items",),
        'task_ids': fields.one2many('project.task', 'project_id',
                                    domain=[('stage_id.fold', '=', False)]),
        'color': fields.integer('Color Index'),
        'alias_id': fields.many2one('mail.alias', 'Alias', ondelete="restrict", required=True,
                                    help="Internal email associated with this project. Incoming emails are automatically synchronized"
                                         "with Line Items (or optionally POs if the Issue Tracker module is installed)."),
...
class task(osv.osv):
    ...
    _columns = {
        ...
        'name': fields.char('SITE ID', track_visibility='onchange', size=128, required=True, select=True),
        ...
        'parent_ids': fields.many2many('project.task', 'project_task_parent_rel', 'task_id', 'parent_id', 'Parent Line Items'),
        'child_ids': fields.many2many('project.task', 'project_task_parent_rel', 'parent_id', 'task_id', 'Delegated Line Items'),
...
class account_analytic_account(osv.osv):
    ...
    _columns = {
        'use_tasks': fields.boolean('Line Items',help="If checked, this contract will be available in the project menu and you will be able to manage tasks or track issues"),
...
class project_task_history(osv.osv):
    """
    Line Items History, used for cumulative flow charts (Lean/Agile)
    """
    _name = 'project.task.history'
    _description = 'History of Line Items'
    _rec_name = 'task_id'
    _log_access = False
...
class project_task_history_cumulative(osv.osv):
    ...
    _columns = {
        ...
        'nbr_tasks': fields.integer('# of Line Items', readonly=True),
