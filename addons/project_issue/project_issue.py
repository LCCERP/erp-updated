...
class project_issue(osv.Model):
    ...
    def _get_issue_task(self, cr, uid, ids, context=None):
        ...
        for task in self.pool.get('project.task').browse(cr, uid, ids, context=context):
            POs += issue_pool.search(cr, uid, [('task_id','=',task.id)])
        return POs
    ...
    def _get_issue_work(self, cr, uid, ids, context=None):
        ...
        for work in self.pool.get('project.task.work').browse(cr, uid, ids, context=context):
            if work.task_id:
                POs += issue_pool.search(cr, uid, [('task_id','=',work.task_id.id)])
        return POs

    _columns = {
        'id': fields.integer('ID', readonly=True),
        'name': fields.char('PO NO', required=True),
        'project_no': fields.integer('Project NO', required=False),
        'index_no': fields.integer('INDEX NO', required=False),
        'po_rel_ver': fields.integer('PO/Rel.Ver', required=False),
        'department': fields.char('Department', required=False),
        'categ_ids': fields.many2many('project.category', string='Region'),
        'issue_date': fields.date('Issue Date'),
        'scope_of_work': fields.char('Scope Of Work', required=False),
        'lcc_responsible_person': fields.many2one('res.users', 'LCC Responsible Person', select=True, track_visibility='onchange'),
        'po_status': fields.char('PO Status', required=False),
        'parent_customer_name':fields.char('Parent Customer Name',    required=False),
        'project_id': fields.many2one('project.project', 'LCC Project Name', track_visibility='onchange', select=True),
        'sub_contract_no':fields.char('Sub Contract No',    required=False),
        'huawei_project_name':fields.char('Huawei Project Name',    required=False),
        'payment_terms':fields.char('Payment Terms',    required=False),
        'start_date': fields.date('Start Date'),
        'end_date': fields.date('End Date'),
        'completion':fields.char('Completion %',    required=False),
        'pac':fields.char('PAC %',    required=False),
        'fac':fields.char('FAC %',    required=False),
        'total_po_amount': fields.integer('Total PO Amount', required=False),
        'tax': fields.integer('Tax', required=False),
        'total_amount_included_tax': fields.integer('Total Amount Included Tax', required=False),
        'remarks': fields.text('Remarks'),
        'line_items': fields.one2many('project.task', 'po_no', 'Line Items'),
        'active': fields.boolean('Active', required=False),
    ...
    def get_empty_list_help(self, cr, uid, help, context=None):
        ...
        context['empty_list_help_document_name'] = _("POs")
    ...
    def message_get_reply_to(self, cr, uid, ids, context=None):
        ...
        POs = self.browse(cr, SUPERUSER_ID, ids, context=context)
        project_ids = set([issue.project_id.id for issue in POs if issue.project_id])
        aliases = self.pool['project.project'].message_get_reply_to(cr, uid, list(project_ids), context=context)
        return dict((issue.id, aliases.get(issue.project_id and issue.project_id.id or 0, False)) for issue in POs)
...
class project(osv.Model):
    ...
    _columns = {
        ...
        'issue_count': fields.function(_issue_count, type='integer', string="POs",),
...
class account_analytic_account(osv.Model):
    ...
    _columns = {
        'use_POs': fields.boolean('POs', help="Check this field if this project manages POs"),
    }
    ...
    def on_change_template(self, cr, uid, ids, template_id, date_start=False, context=None):
        ...
        if template_id and 'value' in res:
            ...
            res['value']['use_POs'] = template.use_POs
        return res
    ...
    def _trigger_project_creation(self, cr, uid, vals, context=None):
        ...
        return res or (vals.get('use_POs') and not 'project_creation_in_progress' in context)
...
class project_project(osv.Model):
    ...
    def _check_create_write_values(self, cr, uid, vals, context=None):
        ...
        # Handle use_tasks / use_POs: if only one is checked, alias should take the same model
        if vals.get('use_tasks') and not vals.get('use_POs'):
            vals['alias_model'] = 'project.task'
        elif vals.get('use_POs') and not vals.get('use_tasks'):
            vals['alias_model'] = 'project.issue'

    def on_change_use_tasks_or_POs(self, cr, uid, ids, use_tasks, use_POs, context=None):
        values = {}
        if use_tasks and not use_POs:
            values['alias_model'] = 'project.task'
        elif not use_tasks and use_POs:
...
class res_partner(osv.osv):
    ...
    _columns = {
        'issue_count': fields.function(_issue_count, string='# POs', type='integer'),
    }
